...
switch(lower(Interpolation))
    case 'nearest'
        xBas0=round(Tlocalx) + ndgrid(1:128,1:128);
        yBas0=round(Tlocaly) + ndgrid(1:128,1:128)';
...
switch(lower(Interpolation))
    case 'nearest'
        ...
        xBas0=min(max(xBas0,1),size(Iin,1));
        yBas0=min(max(yBas0,1),size(Iin,2));
    case 'bilinear'
        check_xBas0=(xBas0<0)|(xBas0>(size(Iin,1)-1));
        check_yBas0=(yBas0<0)|(yBas0>(size(Iin,2)-1));
...
for i=1:lo % Loop incase of RGB
    ...
    switch(lower(Interpolation))
        case 'nearest'
            % Get the intensities
            intensity_xyz0=Iin_one(0+xBas0+(yBas0-1)*size(Iin,1));
