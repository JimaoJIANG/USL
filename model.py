...
class USLModel(nn.Module):
    ...
    def _KFNN(self, x, y, k=10):
        def pairwise_dist(a, b):
            ...
            num_points_y, points_dim = y.size()
            # assert(num_points_x == 1024 or num_points_x == 256)

            xx = torch.pow(x, 2).sum(1)
            yy = torch.pow(y, 2).sum(1)
            zz = torch.mm(x, y.permute(1, 0))
            rx = xx.unsqueeze(0).expand(num_points_y, num_points_x)
            ry = yy.unsqueeze(0).expand(num_points_x, num_points_y)

            P = rx.permute(1, 0) + ry - 2 * zz
            return P

        pairwise_distance = pairwise_dist(x, y)
    ...
    def cal_C(self, fea_spec_1, fea_spec_2):
        ## 闭式解
        # C = torch.matmul(torch.matmul(torch.pinverse(torch.matmul(fea_spec_1, fea_spec_1.T)), fea_spec_1), fea_spec_2.T)
        ## svd
        xx = torch.matmul(fea_spec_1, fea_spec_2.T)
        uu, ss, vv = torch.linalg.svd(xx)
        tempss = (ss <= 1e-4).to(torch.float32)
        ss = tempss * ss + (ss > 1e-4).to(torch.float32)
        C = torch.matmul(torch.matmul(uu, torch.diag(ss)), vv)
        return C

    def cal_C_deep(self, fea_spec_1, fea_spec_2, metric="cosine"):
        if metric == "cosine":
            similarity = torch.matmul(fea_spec_1, fea_spec_2.T) / \
                         (torch.norm(fea_spec_1, p=2) * torch.norm(fea_spec_2, p=2) + 1e-6)
        elif metric == "KFNN":
            pairwise_distance, _ = self._KFNN(fea_spec_1, fea_spec_2)
    ...
    def forward(self, input_1, input_2):
        ...
        if self.method == "deep":
            ...
            c_1 = self.cal_C(fea_spec_1, self.latent_fea)
            c_2 = self.cal_C(fea_spec_2, self.latent_fea)
